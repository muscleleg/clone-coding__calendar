...
async function findHolidaysFromLocal(targetYear) {
    ...
    if (localStorage.getItem("H" + targetYear) == null) {
        ...
        await createHolidaysToLocalStorageWithPromise(targetYear);
    }
    apiLoading = false;
}

async function createHolidaysToLocalStorageWithPromise(year) {
    //   const urls = [
    ...
    // let holidays;
    // for (let i = 0; i < 12; i++) {
    //     holidaysPerMonthPromise.push(getHolidayFromOpenApiWithPromise(year, i));
    // }
    // Promise.all([getHolidayFromOpenApiWithPromise(2022, 0),getHolidayFromOpenApiWithPromise(2022, 1),getHolidayFromOpenApiWithPromise(2022, 2)]).then((data)=>{
    //         console.log(data);
    //     }
    // );
    // let urls = [];
    // for (let i = 0; i < 12; i++) {
    //  urls.push(fetch("https://apis.data.go.kr/B090041/openapi/service/SpcdeInfoService/getHoliDeInfo?serviceKey=ekC4oLj9HiRR136lGjdLiiNDOzqPEKSYXbumkXqOxtHTCnA1pxSmHbI4FacDWPcwui2XiEme6dl9UJT2Bva6fA%3D%3D&solYear=" + year + "&solMonth=" + String(i + 1).padStart(2, "0") + "&_type=json").then(res=>res.json()));
    // }
    const test = await Promise.all([
        ...
        for(let d of data){
            holidaysPerMonth.push(d.response.body);
        }
    }).then((data)=>{
...
function getHolidayFromOpenApiWithPromise(year, month) {
    //promise all 적용
    return fetch(
        "https://apis.data.go.kr/B090041/openapi/service/SpcdeInfoService/getHoliDeInfo?serviceKey=ekC4oLj9HiRR136lGjdLiiNDOzqPEKSYXbumkXqOxtHTCnA1pxSmHbI4FacDWPcwui2XiEme6dl9UJT2Bva6fA%3D%3D&solYear=" +
        year +
